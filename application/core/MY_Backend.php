...
class MY_Backend extends CI_Controller {
    ...
    protected function getAppConfig(){
        ...
        return array(
            ...
            'export_applogo' => FCPATH.'/upload/'.$appSettings['app_logo']
        );
    }
    ...
    protected function getAuthorized(){
        $routeBase = $this->config->item('route_backend').'/login';
    ...
    protected function template($config = array()){
        ...
        //echo '<pre>'.print_r($authorized, 1).'</pre>';
        //$this->load->view($cfgDirView.'template/index', $data);
        if($authorized){
            $this->load->view($cfgDirView.'template/index', $data);
        }
        else{
            $this->load->view($cfgDirView.'login/index', $data);
        }
    }
}
